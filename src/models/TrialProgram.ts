...
export interface ITrialProgram extends Document {
  name: string;
  slug: string;
  description: string;
  shortDescription: string;
  paidProgramId: mongoose.Types.ObjectId;
...
const TrialProgramSchema = new Schema<ITrialProgram>(
  {
    name: {
      type: String,
    ...
    paidProgramId: {
      ...
      ref: 'PaidProgram',
      required: [true, 'Associated paid program is required'],
    },
    entityId: {
      type: Schema.Types.ObjectId,
      ref: 'Entity',
    ...
    price: {
      type: Number,
      required: [true, 'Price is required (can be 0 for free trials)'],
...
// Indexes
TrialProgramSchema.index({ slug: 1 });
TrialProgramSchema.index({ paidProgramId: 1 });
TrialProgramSchema.index({ entityId: 1, categoryId: 1 });
TrialProgramSchema.index({ isActive: 1 });

// Validation: if deliveryFormat is in-person or hybrid, venueAddress is required
TrialProgramSchema.pre('validate', function () {
  if (
...
export default mongoose.model<ITrialProgram>('TrialProgram', TrialProgramSchema);
